...
function predict(draws) {
  ...
  // ── KESİN 6: C8+C7 havuzundan tekrar oranına göre top 6 ──
  // Strateji: C8+C7 birleşik havuzunu tekrar oranına göre sırala, top 6 al
  // Böylece C7'den gelen sayılar da tekrar oranına göre seçilir, küçük sayı sorunu olmaz
  var c8Set = {};
  certain8List.forEach(function(x){ c8Set[x] = 1; });
  var c7Only = result.certain7.filter(function(x){ return !c8Set[x]; });
  // Birleşik havuz: C8 + C7'nin benzersiz sayıları
  var combinedPool = certain8List.concat(c7Only);
  // Tekrar oranına göre sırala
  var poolScored = combinedPool.slice().sort(function(a,b){
    return (REPEAT_RATE[b]||0.72)-(REPEAT_RATE[a]||0.72);
  });
  var certain6List = poolScored.slice(0,6);
