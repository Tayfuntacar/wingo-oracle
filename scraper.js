      ...
      rows.forEach(function(r) {
        ...
        if (pc7.length > 0) {
          ...
          pc7.forEach(function(num) {
            var inIlk6 = af5.indexOf(num) !== -1;
            var inFull = aAll.indexOf(num) !== -1;
            var bg  = inIlk6 ? '#3a3000' : inFull ? '#2a2000' : '#2a3040';
            var brd = inIlk6 ? '#facc15' : inFull ? '#d4a843' : '#4a5270';
            var cl  = inIlk6 ? '#fef08a' : inFull ? '#d4a843' : '#aab0c4';
            h += '<div class="nb" style="background:'+bg+';border:2px solid '+brd+';color:'+cl+'">'+num+'</div>';
          });
          h += '</div>';
          h += '<div style="font-size:10px;color:#5a6180;margin-top:3px;display:flex;gap:10px;flex-wrap:wrap">';
          h += '<span><span style="display:inline-block;width:10px;height:10px;border-radius:50%;background:#3a3000;border:2px solid #facc15;vertical-align:middle"></span> Ilk 6\'da cikti</span>';
