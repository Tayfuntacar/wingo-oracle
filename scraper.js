...
db.connect().then(function() {
  ...
  return db.query('ALTER TABLE predictions ADD COLUMN IF NOT EXISTS pred_certain7 TEXT');
}).then(function() {
  return db.query('ALTER TABLE predictions ADD COLUMN IF NOT EXISTS certain7_match INT DEFAULT -1');
}).then(function() {
...
function startDashboard() {
  ...
  app.get('/rapor', function(req, res) {
    db.query("SELECT p.*,d.all_numbers as actual_all,p.certain7_match FROM predictions p LEFT JOIN draws d ON p.round=d.round WHERE p.ou_hit != -1 AND p.created_at > NOW() - INTERVAL '7 days' ORDER BY p.created_at DESC LIMIT 1000").then(function(result) {
  ...
  app.get('/', function(req, res) {
    ...
    h += 'h+="<div style=\'margin-top:10px\'>";'
    h += 'h+="<div style=\'font-size:10px;color:#aab0c4;margin-bottom:5px\'>Son 10 Tur</div>";'
    h += 'h+="<div style=\'display:flex;flex-wrap:wrap;gap:5px\'>";'
    ...
    h += 'h+="<div style=\'width:32px;height:32px;border-radius:50%;background:"+rbg+";border:"+brd+";display:inline-flex;align-items:center;justify-content:center;font-size:12px;font-weight:800;color:"+tc+"\'>"+(rc10===0?"0":rc10)+"</div>";'
    h += 'h+="<div style=\'font-size:8px;color:#aab0c4\'>"+rcn.slice(0,3)+"</div>";'
    h += 'h+="</div>";'
    h += '});'
    h += 'h+="</div>";'
    h += 'if(zc10>=4){h+="<div style=\'margin-top:5px;padding:3px 8px;background:#2a1500;border-radius:4px\'>"'
