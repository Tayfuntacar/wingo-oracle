...
function predict(draws) {
  ...
  // ── KESİN 6: C8'den 3 + C7'den 3 (S8 stratejisi, 2414 çekiliş test: 6/6=%14.4) ──
  var c8Set = {};
  certain8List.forEach(function(x){ c8Set[x] = 1; });
  ...
  // Eğer c7Only 3'ten azsa c8'den tamamla
  if (certain6List.length < 6) {
    for (var ci=3; ci<certain8List.length && certain6List.length<6; ci++) {
      if (certain6List.indexOf(certain8List[ci]) === -1) certain6List.push(certain8List[ci]);
    }
  }
